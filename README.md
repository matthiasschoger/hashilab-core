...
- csi-nfs - CSI which allows to mount NFS shares from my Synology into Nomad services and assures that only a single alloc is accessing the persistant data on the NAS share.
- csi-iscsi - democratic-CSI to mount iSCSI targets from the Synology NAS. The controller works and can create targets and LUNs via `nomad volume create`. Required for databases which don't like NFS shares.
